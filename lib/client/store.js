...
export default class ClientStore extends Store {
  ...
  getModel (modelId, options = {update: true}) {
    ...
    if (!model) {
      ...
      if (options.update && this.shouldCollectionUpdate()) {
        model.fetch();
      }
    }
  ...
  update (data, deferred) {
    this.getCollection({fetch: false});
    const Model = this.collection.model;
    const id = data[Model.prototype.idAttribute] ? data[Model.prototype.idAttribute] : (Model.prototype.parse(data))[Model.prototype.idAttribute];
